    ...
    void RecorderApplication::drawUI()
    {
        ...
        if (wf_size > 0 && ImGui::BeginTable("##recorder_table", 2, ImGuiTableFlags_NoBordersInBodyUntilResize | ImGuiTableFlags_Resizable | ImGuiTableFlags_SizingStretchProp))
        {
            ...
            ImGui::BeginChild("RecorderChildPanel", {left_width, wf_size}, false, ImGuiWindowFlags_AlwaysVerticalScrollbar);
            {
                ...
                if (ImGui::CollapsingHeader("设备", tracking_started_cli ? ImGuiTreeNodeFlags_None : ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ImGui::Spacing();
                    if (assume_started)
                        style::beginDisabled();
                    ...
                    */
                    ...
                    ImGui::InputInt("抽取###recorderdecimation", &current_decimation);
                    ...
                    if (ImGui::InputDouble("MHz (本振偏移)##downupconverter", &xconverter_frequency))
                        set_frequency(frequency_hz);
                    if (pushed_color_xconv)
                        ImGui::PopStyleColor();
                ...
                {
                    if (ImGui::Combo("FFT 大小", &selected_fft_size, "131072\0"
                                                                     "65536\0"
                                                                     "32768\0"
                                                                     "16384\0"
                                                                     "8192\0"
                    ...
                    if (ImGui::InputInt("FFT 速率", &fft_rate))
                    {
                        if (fft_rate <= 0)
                            fft_rate = old_rate;
                        else
                    ...
                    if (ImGui::InputInt("频谱瀑布速率", &waterfall_rate))
                    {
                        if (waterfall_rate <= 0)
                    ...
                    widgets::SteppedSliderFloat("FFT 最大值", &fft_plot->scale_max, -160, 150);
                    widgets::SteppedSliderFloat("FFT 最小值", &fft_plot->scale_min, -160, 150);
                    widgets::SteppedSliderFloat("平均次数", &fft->avg_num, 1, 500, 1);
                    if (ImGui::Combo("调色板", &selected_waterfall_palette, waterfall_palettes_str.c_str()))
                        waterfall_plot->set_palette(waterfall_palettes[selected_waterfall_palette]);
                    ImGui::Checkbox("显示频谱瀑布", &show_waterfall);
                    ImGui::Checkbox("频率标尺", &fft_plot->enable_freq_scale);
                }
                ...
                if (ImGui::CollapsingHeader("处理"))
                {
                    ...
                    {
                        if (ImGui::BeginCombo("频率###presetscombo", pipeline_selector.selected_pipeline.preset.frequencies[pipeline_preset_id].second == frequency_hz ? pipeline_selector.selected_pipeline.preset.frequencies[pipeline_preset_id].first.c_str() : ""))
                        {
                    ...
                    if (!assume_processing)
                    {
                        if (ImGui::Button("开始###startprocessing"))
                            start_processing();
                    }
                    else if (assume_stopping_processing)
                    {
                        style::beginDisabled();
                        ImGui::Button("正在停止...##stoppingprocessing");
                        style::endDisabled();
                    }
                    else
                    {
                        if (ImGui::Button("停止##stopprocessing"))
                ...
                if (ImGui::CollapsingHeader("录制"))
                {
                    bool assume_recording = is_recording;
                    if (assume_recording)
                        style::beginDisabled();

                    if (baseband_format.draw_record_combo("格式##basebandrecordformat"))
                        file_sink->set_output_sample_type(baseband_format);
                    ...
                    if (file_written < 1e9)
                        ImGui::Text("文件大小：%.2f MB", file_written / 1e6);
                    else
                        ImGui::Text("文件大小：%.2f GB", file_written / 1e9);

                    ImGui::Text("剩余空间：%.2f GB", estimated_available / pow(1024, 3));
...
#endif
                    {
...
#endif
                        {
                            ...
                            ImGui::Text("剩余时间：%02d天%02d:%02d:%02d", day, hour, minutes, seconds);
                        }
                    }
                    ...
                    if (baseband_format == dsp::ZIQ)
                    {
                        if (file_sink->get_written_raw() < 1e9)
                            ImGui::Text("原始大小：%.2f MB", file_sink->get_written_raw() / 1e6);
                        else
                            ImGui::Text("原始大小：%.2f GB", file_sink->get_written_raw() / 1e9);
                    }
#endif

                    ImGui::Text("文件：%s", recorder_filename.c_str());

                    ImGui::Spacing();

                    if (!assume_recording)
                        ImGui::TextColored(style::theme.red, "空闲");
                    else
                        ImGui::TextColored(style::theme.green, "录制中");

                    ImGui::Spacing();

                    if (!assume_recording)
                    {
                        if (ImGui::Button("开始录制###startrecording"))
                            start_recording();
                    }
                    else
                    {
                ...
                show_tracking = ImGui::CollapsingHeader("跟踪", tracking_started_cli ? ImGuiTreeNodeFlags_DefaultOpen : ImGuiTreeNodeFlags_None);
                if (show_tracking)
                {
                    try_init_tracking_widget();
                ...
                if (ImGui::CollapsingHeader("VFO（虚拟频率）"))
                {
                    vfos_mtx.lock();
                    std::string to_delete = "";
                    if (vfo_list.size() == 0)
                    {
                        const char *no_vfo_text = "没有活跃的VFO";
                    ...
                    else
                    {
                        for (auto &vfo : vfo_list)
                        {
                            ...
                            ImGui::PopStyleColor();
                            ImGui::BulletText("频率：%s", format_notated(vfo.freq, "Hz").c_str());
                            if (vfo.selected_pipeline.name != "")
                            {
                                ImGui::BulletText("管道：%s", vfo.selected_pipeline.readable_name.c_str());
                                ImGui::BulletText("目录：%s", vfo.output_dir.c_str());
                            }
                            else if (vfo.file_sink)
                            {
                                if (vfo.file_sink->get_written() < 1e9)
                                    ImGui::BulletText("IQ文件大小：%.2f MB", vfo.file_sink->get_written() / 1e6);
                                else
                                    ImGui::BulletText("IQ文件大小：%.2f GB", vfo.file_sink->get_written() / 1e9);
                            }
                            ImGui::Spacing();
                            if (ImGui::Button(std::string("停止##" + vfo.id).c_str(), ImVec2(ImGui::GetContentRegionAvail().x, 0)))
                                to_delete = vfo.id;
                        }
                    }
                    vfos_mtx.unlock();
