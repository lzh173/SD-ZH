...
namespace satdump
{
    ...
    {
        bool ret = false;
        if(ImGui::CollapsingHeader("导出到CLI", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ImGuiStyle& imgui_style = ImGui::GetStyle();
            initial_frequency.draw();
            if (ImGui::Combo("源设备", &selected_sdr, sdr_sources_str.c_str()))
                source_obj = dsp::dsp_sources_registry[sdr_sources[selected_sdr]]
                    .getInstance({sdr_sources[selected_sdr], "", "" });
            ImGui::InputTextWithHint("源ID", "[自动]", &source_id);
            source_obj->drawControlUI();
            ImGui::Spacing();
            ImGui::Separator();
            ImGui::Spacing();
            ImGui::InputTextWithHint("HTTP服务器", "[禁用]", &http_server);
            ...
            ImGui::Checkbox("启用FFT", &fft_enable);
    ...
    {
        bool ret = false;
        if (ImGui::CollapsingHeader("从CLI导入"))
        {
            import_file.draw();
            ImGui::Checkbox("导入调度器选项", &import_autotrack_settings);
            ImGui::SameLine();
            ImGui::Checkbox("导入旋转器设置", &import_rotator_settings);
            ImGui::SameLine();
            ImGui::Checkbox("导入跟踪对象", &import_tracked_objects);
            ImGui::Spacing();
            if (ImGui::Button("导入"))
                ret = true;
            import_message.draw();
    ...
    void TrackingImportExport::do_export(AutoTrackScheduler &scheduler, ObjectTracker &tracker, std::shared_ptr<rotator::RotatorHandler> rotator_handler)
    {
        ...
        {
            export_message.set_message(style::theme.red, "请先选择有效的输出目录");
            return;
        }
        if (source_obj->get_samplerate() == 0)
        {
            export_message.set_message(style::theme.red, "请先设置有效的采样率");
            return;
        }
        ...
        ui_thread_pool.push([this, exported_config](int) {
            ...
            if (result.result().size() > 0)
            {
                ...
                test_output.close();
                export_message.set_message(style::theme.green, "配置导出成功");
            }
        });
    }
    ...
    void TrackingImportExport::do_import(AutoTrackScheduler &scheduler, ObjectTracker &tracker, std::shared_ptr<rotator::RotatorHandler> rotator_handler)
    {
        ...
        {
            import_message.set_message(style::theme.red, "请先选择有效的导入文件");
            return;
        }
        if (!import_autotrack_settings && !import_tracked_objects && !import_rotator_settings)
        {
            import_message.set_message(style::theme.red, "请至少选择一项要导入的设置");
            return;
        }
        ...
        {
            import_message.set_message(style::theme.red, "导入配置时出错！");
            logger->error("Tracking import error: %s", e.what());
            return;
        }

        import_message.set_message(style::theme.green, "配置导入成功");
    }
}