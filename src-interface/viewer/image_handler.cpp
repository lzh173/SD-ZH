            ...
            if (active_channel_id >= 0)
            {
                ...
                if (products->has_calibation() && products->get_wavenumber(active_channel_id) != -1)
                {
                    ...
                    if (radiance != CALIBRATION_INVALID_VALUE)
                    {
                        ...
                        {
                            ImGui::Text("反照率：%.2f %%", radiance * 100.0);
                        }
                        else
                        {
                            ImGui::Text("辐射率：%.10f", radiance);
                            ImGui::Text("温度：%.2f °C", radiance_to_temperature(radiance, products->get_wavenumber(active_channel_id)) - 273.15);
                        }
                    }
                    else
                    {
    ...
    void ImageViewerHandler::drawMenu()
    {
        if (ImGui::CollapsingHeader("图像"))
        {
            ...
            if (ImGui::IsItemHovered() && active_channel_id >= 0)
            {
                ...
                {
                    ImGui::BeginTooltip();
                    ImGui::Text(u8"\u6ce2\u6570\uff1a%f cm\u207b\u00b9", products->get_wavenumber(active_channel_id));
                    double wl_nm = 1e7 / products->get_wavenumber(active_channel_id);
                    double frequency = 299792458.0 / (wl_nm * 10e-10);

                    if (wl_nm < 1e3)
                        ImGui::Text("波长：%f nm", wl_nm);
                    else if (wl_nm < 1e6)
                        ImGui::Text("波长：%f µm", wl_nm / 1e3);
                    else if (wl_nm < 1e9)
                        ImGui::Text("波长：%f mm", wl_nm / 1e6);
                    else
                        ImGui::Text("波长：%f cm", wl_nm / 1e7);

                    if (frequency < 1e3)
                        ImGui::Text("频率：%f Hz", frequency);
                    else if (frequency < 1e6)
                        ImGui::Text("频率：%f kHz", frequency / 1e3);
                    else if (frequency < 1e9)
                        ImGui::Text("频率：%f MHz", frequency / 1e6);
                    else
                        ImGui::Text("频率：%f GHz", frequency / 1e9);
                    ImGui::EndTooltip();
                }
            }
            ...
            if (products->has_calibation() && active_channel_id >= 0 && products->get_wavenumber(active_channel_id) != -1)
            {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("显示标尺");

                if (range_window && active_channel_calibrated)
                {
                    ImGui::Begin("显示范围控制", &range_window, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
                    ...
                    if (is_temp && products->get_calibration_type(active_channel_id))
                        ...
                        format_string = "%.2f%% \u53cd\u7167\u7387";

                    ImGui::SetNextItemWidth(120 * ui_scale);
                    buff |= ImGui::InputDouble("最小值", &tmp_min, 0, 0, format_string.c_str(), ImGuiInputTextFlags_EnterReturnsTrue);
                    ImGui::SetNextItemWidth(120 * ui_scale);
                    buff |= ImGui::InputDouble("最大值", &tmp_max, 0, 0, format_string.c_str(), ImGuiInputTextFlags_EnterReturnsTrue);
                    if (buff)
                    {
                ...
                if (show_scale && active_channel_calibrated && products->get_calibration_type(active_channel_id))
                {
                    ...
                    if (ImGui::Begin("标尺##scale_window", &show_scale, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize))
                    {
                ...
                if (products->get_calibration_type(active_channel_id))
                {
                    ...
                    if (ImGui::BeginCombo("##temp_rad", is_temp ? "温度" : "辐射率", ImGuiComboFlags_NoArrowButton))
                    {
                        if (ImGui::Selectable("辐射率", !is_temp))
                        {
                            is_temp = false;
                            asyncUpdate();
                        }
                        ...
                        if (ImGui::Selectable("温度", is_temp))
                        {
                            is_temp = true;
                            asyncUpdate();
                        }
            ...
            if (ImGui::Checkbox("中值模糊", &median_blur))
                asyncUpdate();

            if (ImGui::Checkbox("去噪点", &despeckle))
                asyncUpdate();

            if (ImGui::Checkbox("旋转", &rotate_image))
                asyncUpdate();

            if (products->can_geometrically_correct() && ImGui::Checkbox("几何校正", &correct_image))
                asyncUpdate();

            if (products->can_remove_background() && ImGui::Checkbox("去除背景", &remove_background))
                asyncUpdate();

            if (ImGui::Checkbox("直方图均衡", &equalize_image))
                asyncUpdate();

            if (ImGui::Checkbox("逐通道均衡", &individual_equalize_image))
                asyncUpdate();

            if (ImGui::Checkbox("白平衡", &white_balance_image))
                asyncUpdate();

            if (ImGui::Checkbox("归一化", &normalize_image))
                asyncUpdate();

            if (ImGui::Checkbox("反色", &invert_image))
            {
                updateScaleImage();
                asyncUpdate();
            }

            if (ImGui::Checkbox("应用LUT查找表##lutoption", &using_lut))
            {
                asyncUpdate();
                updateScaleImage();
            }

            if (ImGui::Checkbox("手动亮度/对比度", &manual_brightness_contrast))
                asyncUpdate();
            if (manual_brightness_contrast)
            {
                if (ImGui::SliderFloat("亮度", &manual_brightness_contrast_brightness, -2, 2))
                    asyncUpdate();
                if (ImGui::SliderFloat("对比度", &manual_brightness_contrast_contrast, -2, 2))
                    asyncUpdate();
            }
        ...
        if (ImGui::CollapsingHeader("RGB合成"))
        {
                                  ...
                                  select_rgb_presets == -1 ? "" : rgb_presets[select_rgb_presets].first.c_str()))
            {
                ...
                ImGui::InputTextWithHint("##searchpresets", u8"\uf422   \u641c\u7d22", &preset_search_str);
            ...
            if (show_info_button)
            {
                ...
                if (ImGui::Button(u8"\uf449 \u4fe1\u606f###compopresetinfo"))
                {
            ...
            if (rgb_processing)
                style::beginDisabled();
            if (ImGui::Button("应用") && !rgb_processing)
            {
                updateRGB();
        ...
        if (products->has_proj_cfg())
        {
            ...
            if (ImGui::CollapsingHeader("投影"))
            {
                ...
                proj_notif.draw();
                ImGui::Checkbox("旧算法", &projection_use_old_algo);
                if (!canBeProjected())
                    style::endDisabled();
                ...
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled) && !canBeProjected())
                {
                    ...
                    if (current_timestamps.size() == 0)
                        ImGui::TextColored(style::theme.red, "没有时间戳！");
                    else if (correct_image)
                        ImGui::TextColored(style::theme.red, "请禁用几何校正！");
                    else
                        ImGui::TextColored(style::theme.yellow, "旧算法能更好地处理\n"
                                                                "质量很差（高噪声）的数据。\n"
                                                                "不过如果可能的话，\n"
                                                                "首选新算法，\n"
                                                                "因为结果要好得多！:-)\n"
        ...
        if (show_markdown_description)
        {
            ...
            ImGui::Begin("合成信息", &show_markdown_description, ImGuiWindowFlags_NoSavedSettings);
