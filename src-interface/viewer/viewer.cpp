...
namespace satdump
{
    ...
    void ViewerApplication::drawPanel()
    {
        if (ImGui::BeginTabBar("Viewer Prob Tabbar", ImGuiTabBarFlags_None))
        {
            ...
            if (ImGui::BeginTabItem("产品###productsviewertab"))
            {
                if (current_selected_tab != 0)
                    current_selected_tab = 0;

                if (ImGui::CollapsingHeader("常规", ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ...
                    {
                        ImGui::TreeNodeEx("其他", ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen);
            ...
            ImGui::SetNextItemWidth(ImGui::GetWindowWidth() / 2);
            if (ImGui::BeginTabItem("投影###projssviewertab"))
            {
