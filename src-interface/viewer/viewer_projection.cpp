...
namespace satdump
{
    ...
    void ViewerApplication::drawProjectionPanel()
    {
        ...
        if (ImGui::CollapsingHeader("投影", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ImGui::Text("输出图像：");
            ...
            ImGui::Combo("##targetproj", &projections_current_selected_proj, "等距圆柱投影\0"
                                                                             "UTM（墨卡托）\0"
                                                                             "立体投影\0"
                                                                             "卫星透视投影\0"
            ...
            if (projections_current_selected_proj == 0)
            {
                ...
                {
                    ImGui::Text("左上角坐标：");
                    ImGui::InputFloat("纬度##tl", &projections_equirectangular_tl_lat);
                    ImGui::InputFloat("经度##tl", &projections_equirectangular_tl_lon);
                    ImGui::Spacing();
                    ImGui::Text("右下角坐标：");
                    ImGui::InputFloat("纬度##br", &projections_equirectangular_br_lat);
                    ImGui::InputFloat("经度##br", &projections_equirectangular_br_lon);
                }
            }
            else if (projections_current_selected_proj == 1)
            {
                ImGui::InputInt("UTM 带号###projutmzone", &projections_utm_zone);
                if (projections_utm_zone > 60)
                    projections_utm_zone = 60;
                if (projections_utm_zone < 1)
                    projections_utm_zone = 1;
                ImGui::Checkbox("南半球###projutmsouth", &projections_utm_south);
                ImGui::InputFloat("北向偏移 (m)##utm", &projections_utm_offset_y);
                ImGui::Spacing();
                ImGui::InputFloat("比例尺 (m/像素)##utm", &projections_utm_scale);
            }
            else if (projections_current_selected_proj == 2)
            {
                ImGui::Text("中心坐标：");
                ImGui::InputFloat("纬度##stereo", &projections_stereo_center_lat);
                ImGui::InputFloat("经度##stereo", &projections_stereo_center_lon);
                ImGui::Spacing();
                ImGui::InputFloat("比例尺 (m/像素)##stereo", &projections_stereo_scale);
            }
            else if (projections_current_selected_proj == 3)
            {
                ImGui::Text("中心坐标：");
                ImGui::InputFloat("纬度##tpers", &projections_tpers_lat);
                ImGui::InputFloat("经度##tpers", &projections_tpers_lon);
                ImGui::Spacing();
                ImGui::InputFloat("高度 (m)##tpers", &projections_tpers_alt);
                ImGui::InputFloat("倾斜角##tpers", &projections_tpers_ang);
                ImGui::InputFloat("方位角##tpers", &projections_tpers_azi);
                ImGui::Spacing();
                ImGui::InputFloat("比例尺##tpers", &projections_tpers_scale);
            }
            ...
            {
                ImGui::Checkbox("自动模式###pojautomode", &projection_auto_mode);
                ImGui::Checkbox("自动缩放##projautoscalemode", &projection_auto_scale_mode);
                if (projection_auto_scale_mode)
                {
                    ImGui::InputDouble("X 比例尺 (m/像素)##projscalexauto", &projection_autoscale_x);
                    ImGui::InputDouble("Y 比例尺 (m/像素)##projscalexauto", &projection_autoscale_y);
                }
            }
            ...
            {
                if (projections_are_generating)
                    ImGui::SetTooltip("正在生成，请稍候...");
                if (projection_layers.size() == 0)
                    ImGui::SetTooltip("没有加载图层！");
            }

            ImGui::Spacing();

            if (ImGui::Button("保存投影图像"))
            {
        ...
        if (ImGui::CollapsingHeader("图层"))
        {
            ...
            if (ImGui::Button("添加图层##button"))
                ImGui::OpenPopup("添加图层##popup", ImGuiPopupFlags_None);

            if (disable_add_layer)
                style::endDisabled();

            {
                if (ImGui::BeginPopupModal("添加图层##popup", NULL, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize))
                {
                    ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
                    ImGui::RadioButton("等距圆柱投影", &selected_external_type, 0);
                    ImGui::RadioButton("瓦片地图(OSM)", &selected_external_type, 1);
                    ImGui::RadioButton("GeoTIFF", &selected_external_type, 2);
                    ImGui::RadioButton("其他", &selected_external_type, 3);

                    if (selected_external_type == 0 ||
                        selected_external_type == 2 ||
                        selected_external_type == 3)
                    {
                        ImGui::InputText("名称", &projection_new_layer_name);
                        projection_new_layer_file.draw("输入图像");
                        if (selected_external_type == 3)
                            projection_new_layer_cfg.draw("投影配置文件");
                        ImGui::Checkbox("归一化###normalizeinput", &projection_normalize_image);
                    }
                    else if (selected_external_type == 1)
                    {
                        ImGui::InputDouble("纬度1##osmlat1", &projection_osm_lat1);
                        ImGui::InputDouble("经度1##osmlat1", &projection_osm_lon1);
                        ImGui::InputDouble("纬度2##osmlat1", &projection_osm_lat2);
                        ImGui::InputDouble("经度2##osmlat1", &projection_osm_lon2);
                        ImGui::SliderInt("缩放级别##osmsliderzoom", &projection_osm_zoom, 0, 6);
                        if (!urlgood)
                            ImGui::PushStyleColor(ImGuiCol_Text, style::theme.red.Value);

                        ImGui::InputText("瓦片URL", &mapurl, ImGuiInputTextFlags_None);

                        if (!urlgood)
                        {
                            if (ImGui::IsItemHovered())
                                ImGui::SetTooltip("无效URL！格式必须为 https://服务器/{z}/{x}/{y}.ext");
                            ImGui::PopStyleColor();
                        }
                    }
            ...
            if (ImGui::BeginListBox("##projectionslistbox", ImVec2(ImGui::GetWindowWidth(), 300 * ui_scale)))
            {
                for (int i = 0; i < (int)projection_layers.size(); i++)
                {
                    ...
                    if (layer.enabled)
                    {
                        ...
                        if (projections_mode_radio == 0)
                        {
                            ImGui::SetItemTooltip("%s", "透明度仅叠加模式可用");
                            style::endDisabled();
                        }
        ...
        if (ImGui::CollapsingHeader("覆盖##viewerpojoverlay"))
        {
