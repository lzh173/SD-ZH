...
namespace satdump
{
    ...
    void PipelineUISelector::renderSelectionBox(double width)
    {
        ...
        ImGui::InputTextWithHint("##pipelinesearchbox", u8"\uf422   \u641c\u7d22\u7ba1\u9053", &pipeline_search_in);
        if (width != -1)
            ImGui::SetNextItemWidth(width);
        ...
        if (ImGui::BeginListBox("##pipelineslistbox"))
        {
            ...
            {
                if (ImGui::CollapsingHeader("收藏夹", ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ImGui::Spacing();
    ...
    void PipelineUISelector::drawMainparams()
    {
        ...
        {
            int label_width = ImGui::CalcTextSize("输出目录").x;
