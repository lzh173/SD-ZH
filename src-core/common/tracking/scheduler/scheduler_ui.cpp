...
namespace satdump
{
    void AutoTrackScheduler::renderAutotrackConfig(double curr_time)
    {
        ...
        ImGui::InputTextWithHint("##trackingavailablesatssearch", u8"\uf422   \u641c\u7d22\u6240\u6709\u536b\u661f", &availablesatssearch);
        ImGui::BeginGroup();
        ImGui::SetNextItemWidth(200 * ui_scale);
        ...
        ImGui::SetNextItemWidth(200 * ui_scale);
        ImGui::InputTextWithHint("##trackingselectedsatssearch", u8"\uf422   \u641c\u7d22\u5df2\u9009", &selectedsatssearch);
        ...
        ImGui::BeginGroup();
        ImGui::SeparatorText("调度器选项");
        ImGui::Checkbox("多模式", &autotrack_cfg.multi_mode);
        ImGui::SameLine();
        elev_width = ImGui::GetCursorPosX() - elev_width;
        ImGui::Checkbox("空闲时停止SDR", &autotrack_cfg.stop_sdr_when_idle);
        elev_width += ImGui::GetItemRectSize().x - ImGui::CalcTextSize("最小仰角").x - imgui_style.ItemInnerSpacing.x;
        ImGui::Checkbox("显示本地时间", &autotrack_cfg.use_localtime);
        ImGui::SetItemTooltip("%s", "仅在调度器UI中以本地时间显示时间。\n所有其他时间均使用UTC。");
        ImGui::SetNextItemWidth(elev_width);
        ImGui::InputFloat("最小仰角", &autotrack_cfg.autotrack_min_elevation);
        ImGui::EndGroup();
        ImGui::SetCursorPosY(selection_box_bottom);

        ImGui::Separator();
        ImGui::Spacing();
        if (ImGui::Button("更新过境"))
        {
        ...
        if (autotrack_engaged)
            style::endDisabled();
        ImGui::SameLine();
        bool set_engaged = ImGui::Checkbox("启用自动跟踪", &autotrack_engaged);
        ...
        if (ImGui::BeginTable("##trackingwidgettrackeobjectsconfig", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingFixedFit))
        {
            ...
            ImVec2 min_el_size(ImGui::CalcTextSize("最小仰角").x + imgui_style.ItemInnerSpacing.x + (60.0f * ui_scale), 0.0f);
            float downlink_dd_width = (300 * ui_scale) -
                ImGui::CalcTextSize("配置").x - imgui_style.FramePadding.x * 2 - imgui_style.ItemSpacing.x;
            for (auto &cpass : enabled_satellites)
            {
                ...
                for (auto &downlink : cpass.downlinks)
                {
                    ...
                    if (&downlink == &cpass.downlinks[0] && (sat_row == hovered_sat || cpass.min_elevation > 0))
                    {
                        ...
                        ImGui::InputFloat(((std::string) "最小仰角##objdelminelevation" + idpart).c_str(), &cpass.min_elevation);
                    ...
                    std::string modal_title = "配置 " + object_name + " - " + downlink.pipeline_selector->selected_pipeline.readable_name +
                    ...
                    ImGui::SetNextItemWidth(100 * ui_scale);
                    ImGui::Checkbox(((std::string) "录制##objcfgfreq2" + idpart).c_str(), &downlink.record);
                    // ImGui::TableSetColumnIndex(3);
                    ImGui::Checkbox(((std::string) "实时##objcfgfreq3" + idpart).c_str(), &downlink.live);
                    ImGui::TableSetColumnIndex(3);
                    ...
                    ImGui::SameLine();
                    if (ImGui::Button(((std::string)"配置##" + idpart).c_str()))
